...
@section('content')
    ...
    <div class="card border-top border-0 border-4 border-primary">
        <div class="card-body">
            ...
            <form action="{{ route('review-store') }}" method="POST" class="row g-3" enctype="multipart/form-data">
                @csrf
                ...
                <div class="col-md-3">
                    <label for="url" class="form-label">Video URL <code>*</code></label>
                    <input type="url" name="url" class="form-control @error('title') is-invalid @enderror" placeholder="Enter url" id="video" value="{{ old('url') }}">
                    @error('url')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
