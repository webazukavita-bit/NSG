            ...
            <form action="{{ route('blog-update',['id' => $data->id]) }}" method="POST" class="row g-3" enctype="multipart/form-data">
                ...
                <div class="col-md-3">
                    ...
                    <select name="status" class="single-select @error('status') is-invalid @enderror" id="status">
                        <option selected disabled value="">Choose...</option>
                        <option value="Pending" @selected('Pending' == old('category_id', $data->status))>Pending</option>
                        <option value="Publish" @selected('Publish' == old('category_id', $data->status))>Publish</option>
                        <option value="Schedule" @selected('Schedule' == old('category_id', $data->status))>Schedule</option>
                        <option value="Unpublish" @selected('Unpublish' == old('category_id', $data->status))>Unpublish</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
