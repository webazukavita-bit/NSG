    ...
    public function index()
    {
        ...
        if (Auth::user()->role_id !== 4) {
            ...
            if ($user->roles->type == "Admin") {
                ...
                $totalRevenue = Order::where('payment_status_id', 2)->sum('final_amount_with_tax');
                $totalRevenueLast = Order::where('payment_status_id', 2)->where('created_at', '>=', now()->subWeek())->sum('final_amount_with_tax');
                ...
                $totalRevenue = Order::where('order_by_id', $user->id)->where('payment_status_id', 2)->sum('final_amount_with_tax');
                $totalRevenueLast = Order::where('order_by_id', $user->id)->where('payment_status_id', 2)->where('created_at', '>=', now()->subWeek())->sum('final_amount_with_tax');
...
=======
        ...
        $order = Order::with(['user', 'status', 'paymentStatus'])->where('order_by_id', $user->id)->latest()->limit(10)->get();
        $orderCount = Order::where('order_by_id', $user->id)->count();
        $todayorder = Order::where('order_by_id', $user->id)->whereDate('created_at', today())->count();
        return view('front.Myprofile', compact('orderCount', 'todayorder', 'order'));
>>>>>>> 360ddc26dbad4cd0f3cad7bb9cc5963e72781074
    }
}
